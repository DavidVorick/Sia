...
We now apply some statistical analysis to determine how certain we can be about a hosts honesty for a given number of responses.
For this analysis, we assume that the host is storing a minimum of 2\^33 indices, and is computing over a hash space of 2\^256 possible values.
2\^33 indicies can be stored on average in 4 bytes (assuming maximum compression), which is why we have chosen this volume of indicies.
Storing 2\^33 indicies in a trivially lightweight lookup table at 4 bytes per index will incur a storage cost of 32 GiB.
We therefore are assuming that the minimum storage under this scheme is 32 GiB.

I don't know what equations to use to figure this out.
I might end up using a simulation.
