...
\begin{enumerate}
	...
	\item When a host leaves a quorum, two additional hosts from two random quorums are swapped with two random hosts from two different random quorums on the network. (network turbulence)
...
When the attacker leaves a quorum that is not the target quorum and then joins again, the effect is:
% EQUATION
Assuming that the attacker controls 50\% of the network.
% EQUATION
...
Because we define a quorum as honest if only 15\% of hosts in the quorum are honest, we can see that even under attack from a 50\% strong attacker, the equilibrium is above that number for target quorums.
%
%
%
%
