...
% Explain what a host is, what they store, etc. So, they have a root hash of the files that every other host is storing, for example. They have a root hash of the seed for proof-of-capacity. They store the block history of their quorum and of potentially other quorums as well.
...
% A diagram here to illustrate which blocks hosts keep.
...
The goal of the network turbulence is to make sure that any amount of rerolling and honest assassinations will still result in an attacker having an equilibrium at 75\% control, which is a full 10\% away from the 85\% required to acheive a dishonest quorum.
...
In the event that an attacker is targeting a quorum or set of quorums to corrupt them, each time they join the network there is a $\frac{1}{s}$ chance of the attacker landing in the target quorum, where $s$ is the number of quorms times the size of the set of quorums being attacked.
The gain for the attacker is then $\frac{1}{s}$.
When the attacker is replaced in the quorum that the attacker left, there is a $\frac{1}{s}$ chance that a host will be pulled from the target quorum, and a 75\% chance that the host pulled will belong to the attacker, assuming that the attacker is targeting a quorum which they already control 75\% of.
The loss for the attacker is then $(\frac{1}{s})(0.75)$.
...
The other quorum will be random, which means there will be an equal chance of getting a malicious host vs. honest host.
The gain is then $(2)(\frac{1}{s})(\frac{1}{2})(\frac{1}{4})$.
The loss is $(2)(\frac{1}{s})(\frac{1}{2})(\frac{3}{4})$.
Assuming that the host leaves a quorum which it has nearly or exactly 75\% control of (but is not the target quorum), the quorum turbulence will cancel out, as the probability is of swapping hosts from two quorums with the same ratios.
We note that in all components, $\frac{1}{s}$ will cancel out, and can be reduced to 1.
Put together, we get the expected change in host count during a reroll to be:
\begin{equation}
...
% At no point should the paper ever be wrong.
...
% Comment overview:
% Send a transaction, plus an indication of which level it skips over to the other side of the quorum.
